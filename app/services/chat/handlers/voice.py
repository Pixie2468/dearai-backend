...
# from app.services.context import summary_context_provider
from app.services.conversations.service import add_message, get_conversation_messages

# from app.services.emotion import hume_detector
# from app.services.guardrails import input_guardrails, output_guardrails
from app.services.llm import LLMMessage, get_llm
from app.services.speech import get_stt, get_tts

SYSTEM_PROMT = (
    "You are a helpful assistant. Keep all responses under 2500 characters. Use plain text only"
    "with no formatting characters such as asterisks, parentheses, brackets, pound signs, dashes"
    "used as bullets, or markdown of any kind. Write in natural spoken sentences as if your"
    "response will be read aloud. No lists, no headers, no bold, no italics. Just clean prose."
)
...
async def handle_voice_chat(
    ...
    try:
        # 1. Transcribe audio
        stt = get_stt()
        transcript = await stt.transcribe(audio_data, language="unknown")
        ...
        # 7. Send to LLM
        llm = get_llm()
        response_content = await llm.chat(llm_messages, system_prompt=SYSTEM_PROMT)
        ...
        # 9. Convert to speech
        tts = get_tts()
        audio_response = await tts.synthesize(response_content, "shubh", "hi-IN")
        ...
        await add_message(
            db, conversation_id, user_id, "user", transcript, "voice", metadata=metadata
        )
