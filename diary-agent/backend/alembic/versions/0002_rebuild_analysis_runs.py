...
output_json, etc.) and recreates it with the current schema: chat_history,
...
def downgrade() -> None:
    ...
    op.create_table(
        "analysis_runs",
        sa.Column("id", postgresql.UUID(as_uuid=True), primary_key=True),
        sa.Column("chat_history", sa.String(2000), nullable=False),
        ...
        sa.Column("overall_emotional_peak_strength", sa.Float(), nullable=False),
        sa.Column("overall_reflection_depth", sa.Float(), nullable=False),
        sa.Column(
            "created_at",
            sa.DateTime(timezone=True),
            nullable=False,
